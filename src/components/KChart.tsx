...
export function KChart({ bars, upColor = '#22c55e', downColor = '#ef4444', height = 420, tooltip = true, indicators = DEFAULT_INDICATORS }: Props) {
  ...
  // Build the single chart. pane 0 = candles + EMA; pane 1 = MACD (shared time axis).
  useEffect(() => {
    ...
    const chart = createChart(container, {
      ...
      height: macdEnabled ? height + 110 : height,
      layout: {
        background: PANEL_BG,
        textColor: '#8b96a8',
    ...
    // MACD pane (pane 1) — shares the same timeScale as pane 0.
    if (macdEnabled) {
  ...
  }, [height, upColor, downColor, tooltip, macdEnabled]);
  ...
  }, [bars, indicators.ema.enabled, indicators.ema.period, indicators.macd.enabled, indicators.macd.fast, indicators.macd.slow, indicators.macd.signal]);
